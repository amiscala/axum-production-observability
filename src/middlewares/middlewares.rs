...
pub async fn logging_middleware(
    ...
    let string_body = String::from_utf8(body_bytes.clone().into()).unwrap_or_else(|e| {
        format!("{:?}",e)
    });
    ...
    map_and_log_headers(&request_span, request.headers(), opentelemetry_semantic_conventions::trace::HTTP_REQUEST_HEADER);
    ...
    map_and_log_headers(&request_span, response.headers(), opentelemetry_semantic_conventions::trace::HTTP_RESPONSE_HEADER);
    ...
    let response_body_string = String::from_utf8(response_body_bytes.clone().into()).unwrap_or_else(|e| {
        format!("{:?}",e)
    });
    ...
    let response = Response::from_parts(response_parts, axum::body::Body::from(response_body_bytes));
    response
}
...
fn make_request_span(request: &Request) -> Span {
    ...
    let formated_port = if port.is_empty() { "".to_string() } else { format!(":{}", port) };
    let full_uri = format!("{}://{}{}{}", scheme, server_address, &formated_port, query_and_path);
    let real_ip_header_value = match request.headers().get("X-Real-Ip") {
        Some(x_real_ip) => x_real_ip.to_str().unwrap_or_else(|_e| {
            "UNKNOWN"
        }),
        None => "NOT_SENT"
    };
    let user_agent = match request.headers().get("User-Agent") {
        Some(user_agent) => user_agent.to_str().unwrap_or_else(|_e| {
            "UNKNOWN"
        }),
        None => "NOT_SENT"
    };
    ...
    let request_span = info_span!("RequestSummary",
...
fn map_and_log_headers(span: &Span, headers: &axum::http::HeaderMap<axum::http::HeaderValue>, header_name_prefix: &str) {
    ...
    for (header_name, header_value) in headers {
        if let Ok(header_value_str) = header_value.to_str() {
            ...
            if let Some(header_val) = headers_dict.get_mut(header_name.as_str()) {
                println!("{}",header_name);
